...
export default function App() {
  ...
  // Get performance data for AI insights
  ...
  // Get appropriate data for AI Insights based on active tab
  const getAIInsightsData = () => {
    const playerName = selectedPlayer?.name;
    
    // Helper to filter entries by player
    ...
    switch (activeTab) {
      case 'batting':
        return { entries: filterByPlayer(battingEntries), gameStats, playerName };
      case 'pitching':
        return { entries: filterByPlayer(pitchingEntries), gameStats, playerName };
      case 'running':
        return { entries: filterByPlayer(runningEntries), gameStats, playerName };
      case 'strength':
        return { entries: filterByPlayer(strengthEntries), gameStats, playerName };
      case 'exercise':
        return { entries: filterByPlayer(exerciseEntries), gameStats, playerName };
      case 'gamestats':
        return { entries: [], gameStats, playerName };
      default:
        return null;
    }
  };
    ...
    <div className="min-h-screen bg-[#0a0f1a] text-foreground">
      ...
      >
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[11px] font-semibold text-[#38bdf8] tracking-[1.5px] uppercase mb-1">
                Youth Performance Tracking System
              </p>
              <h1 className="text-[18px] md:text-[22px] font-bold text-[#f1f5f9] mb-1">
                PREMIER SELECT
              </h1>
              <p className="text-[11px] md:text-[12px] text-[#64748b] hidden md:block">
                Track baseball player KPIs, benchmarks, and goals across all performance dimensions
              </p>
            </div>
            ...
            <Button
              ...
              variant="outline"
              className="md:hidden border-[#38bdf8] text-[#38bdf8] hover:bg-[#0c4a6e]"
            >
      ...
      {menuOpen && (
        <div className="fixed inset-0 bg-black/80 z-50 md:hidden" onClick={() => setMenuOpen(false)}>
          <div 
            className="bg-[#0f172a] w-72 h-full border-r border-[#1e293b] p-6"
              ...
              {tabs.map(tab => (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-left transition-colors ${
                    activeTab === tab.value
                      ? 'bg-[#0c4a6e] text-[#38bdf8] border border-[#38bdf8]'
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-4 md:py-8">
        {/* Desktop Navigation + Current Tab Display */}
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          ...
          <div className="md:hidden flex items-center gap-3 px-4 py-3 bg-[#0f172a] border border-[#1e293b] rounded-lg">
            {activeTabInfo && (
              <>
                <activeTabInfo.icon className="h-5 w-5 text-[#38bdf8]" />
                <span className="text-[14px] font-medium text-[#f1f5f9]">{activeTabInfo.label}</span>
              </>
            )}
          </div>

          {/* Player Selector (when needed) */}
        ...
        {/* AI Insights - Shows for performance tabs */}
        {aiInsightsData && (
          <div className="mt-6">
            <AIInsights
