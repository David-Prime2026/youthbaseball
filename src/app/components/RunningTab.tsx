...
import { DRILL_OPTIONS, RUNNING_BENCHMARKS } from '../data/benchmarks';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from './ui/select';
import { Label } from './ui/label';
import { AGE_GROUPS } from '../types/player';

export function RunningTab() {
  const { entries, players, addEntry, deleteEntry, addPlayer } = usePerformanceData('running');
  const { addPlayer: addPlayerToDatabase } = usePlayerData();
  const [selectedMetric, setSelectedMetric] = useState('30-Yard Dash');
  ...
  // For running, lower is better, so use Math.min for PR
  const personalRecord = allReps.length > 0 ? Math.min(...allReps) : 0;
  ...
  return (
    <div className="space-y-6">
        ...
        <div>
          ...
          <Select value={selectedMetric} onValueChange={setSelectedMetric}>
            ...
            <SelectContent className="bg-[#1e293b] border-[#334155]">
              <SelectItem value="30-Yard Dash" className="text-[#e2e8f0]">30-Yard Dash</SelectItem>
              <SelectItem value="Home-to-First" className="text-[#e2e8f0]">Home-to-First</SelectItem>
              <SelectItem value="60-Yard Dash" className="text-[#e2e8f0]">60-Yard Dash</SelectItem>
            </SelectContent>
          </Select>
        </div>
        <div>
      ...
      <DataEntryForm
        ...
        metricType={selectedMetric}
        metricUnit={metricUnits[selectedMetric]}
        drillOptions={DRILL_OPTIONS.running}
        players={players}
